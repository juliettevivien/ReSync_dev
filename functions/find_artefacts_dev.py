...
def find_LFP_sync_artefact(
    ...
    assert use_kernel in ['1', '2','3'], 'use_kernel incorrect'
    ...
    kernels = {'1': np.array([1, -1]),
               ...
               '3': np.array([0, 1, -2] + list(np.linspace(-2, 0, 20)))}
    ker = kernels[use_kernel]
    
    # check whether signal is inverted
    max_lfp = max(lfp_data)
    print(f'max: {max_lfp}')
    min_lfp = min(lfp_data)
    print(f'min: {min_lfp}')
    print(f'mean: {np.mean(lfp_data)}')

    
    for i in range(0,len(lfp_data)):
        if lfp_data[i] > max_lfp*0.75 and (all(lfp_data[(i+1):(i+400)]) > np.mean(lfp_data)): 
            first_positive_peak = i
            print(f'first_positive_peak: {first_positive_peak}, value: {lfp_data[i]}')
            break 
    for i in range(0,len(lfp_data)):
        if lfp_data[i] < min_lfp*0.75:
            first_negative_peak = i
            print(f'first_negative_peak: {first_negative_peak}, value: {lfp_data[i]}')
            break
    
    
    if first_positive_peak < first_negative_peak:
    ...
    # revert kernels if signal is inverted:
    if signal_inverted:
        kernels = {'1': (np.array([1, -1]))*(-1),
               '2': (np.array([1, 0, -1] + list(np.linspace(-1, 0, 20))))*(-1),
               '3': (np.array([0, 1, -2] + list(np.linspace(-2, 0, 20))))*(-1)}
        ker = kernels[use_kernel]
    ...
    # use peak of kernel dot products    S
    ...
    if not signal_inverted:
        stim_idx = neg_idx  # this is for 'normal' signal
